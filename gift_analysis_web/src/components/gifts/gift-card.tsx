import { ArrowRight, Layers3, Star, Wallet } from 'lucide-react';

import { formatPrice, formatUsd, resolveAssetPath } from '../../lib/format';
...
export function GiftCard({ gift, badgeDefinition, onReview, onGameplayClick }: GiftCardProps) {
  ...
  const usdLabel = gift.estimatedUsdLabel ?? formatUsd(gift.estimatedUsd);
  const rechargeTooltip = gift.rechargePlanLabel ?? '金币礼物，暂不换算';

  return (
    <article className="glass-panel group flex flex-col overflow-hidden rounded-[30px] border border-white/10 transition duration-300 hover:-translate-y-1 hover:border-cyan/40 hover:shadow-glow">
      ...
      <div className="flex flex-1 flex-col gap-3 p-5">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="rounded-2xl bg-white/6 px-3 py-2 text-right">
            ...
            <p
              className="mt-1 cursor-help text-xs text-slate-300 underline decoration-dotted underline-offset-4"
              title={rechargeTooltip}
              aria-label={`约合美元 ${usdLabel}。${rechargeTooltip}`}
            >
              ≈ {usdLabel}
            </p>
          </div>
        ...
        <div className="mt-auto rounded-[20px] border border-white/10 bg-slate-950/35 p-4">
          <div className="flex items-center justify-between gap-3 text-sm text-slate-300">
            ...
            <div className="inline-flex items-center gap-1 text-cyan-100">
              <Wallet className="h-4 w-4" />
              <span>{gift.currency === 'Diamond' ? '支持充值估值' : '非钻石礼物'}</span>
            </div>
          </div>
